...
void BNReluConvolution6(
  ...
  DMA_copy_k.loc = (uint32_t) l1_buffer + 17144;
  DMA_copy_k.number_of_2d_copies = 1;
  DMA_copy_k.number_of_1d_copies = 1;
  DMA_copy_k.length_1d_copy = (uint16_t) 48;
  dory_dma_memcpy_async(&DMA_copy_k);
  dory_dma_barrier(&DMA_copy_k);


  DMA_copy_lambda.ext = (uint32_t) l2_W+74240;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 17248;
  DMA_copy_lambda.number_of_2d_copies = 1;
  DMA_copy_lambda.number_of_1d_copies = 1;
  DMA_copy_lambda.length_1d_copy = (uint16_t) 48;
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 3312) + 0;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 1600 : 0;
    db_W = !db_state_W ? 6912 : 0;
    db_y = !db_state_y ? 48 : 0;
    db_act = !db_state_W ? 48 : 0;
    exec_db_x = db_state_x ? 1600 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 6912 : 0;
    exec_db_act = db_state_W ? 48 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      x_tile_size_w   = (_i_w_load+1 == 3)   ? 3 : 5;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 5, 5, 64, 10, 64,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 12, 3*3, 64, 3*3, 64, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 3312) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+73728 + 48*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 17144 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+74240 + 48*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 17248 + db_act;
    ...
    k = (int32_t *) (l1_buffer + 17144 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 17248 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 3312 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 3208 + db_y);
    ...
    x_tile_size_w_exec   = (_i_w_exec+1 == 3)   ? 3 : 5;
    y_tile_size_nof = (_i_nof_exec+1 == 11) ? 8 : 12;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 11)   ? 8 : 12;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 2, 2, 12, 5, 128, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 3208) + db_y;
