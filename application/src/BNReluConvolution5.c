...
void BNReluConvolution5(
  ...
  DMA_copy_k.loc = (uint32_t) l1_buffer + 15960;
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+37120;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 16032;
  ...
  DMA_copy_x.number_of_1d_copies = 8;
  DMA_copy_x.length_1d_copy = 64;
  dory_dma_memcpy_async(&DMA_copy_x);
  dory_dma_barrier(&DMA_copy_x);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 6736) + 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 6, 8, 64, 10, 64,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 8, 3*3, 64, 3*3, 64, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 6736) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+36864 + 32*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 15960 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+37120 + 32*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 16032 + db_act;
    ...
    k = (int32_t *) (l1_buffer + 15960 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 16032 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 6736 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 6152 + db_y);
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 6, 6, 8, 10, 64, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 6152) + db_y;
