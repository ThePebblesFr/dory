...
void BNReluConvolution3(
  ...
  DMA_copy_k.loc = (uint32_t) l1_buffer + 21784;
  DMA_copy_k.number_of_2d_copies = 1;
  DMA_copy_k.number_of_1d_copies = 1;
  DMA_copy_k.length_1d_copy = (uint16_t) 112;
  dory_dma_memcpy_async(&DMA_copy_k);
  dory_dma_barrier(&DMA_copy_k);


  DMA_copy_lambda.ext = (uint32_t) l2_W+9344;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 22016;
  DMA_copy_lambda.number_of_2d_copies = 1;
  DMA_copy_lambda.number_of_1d_copies = 1;
  DMA_copy_lambda.length_1d_copy = (uint16_t) 112;
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 5648) + 0;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_W = !db_state_W ? 8064 : 0;
    db_y = !db_state_y ? 896 : 0;
    db_act = !db_state_W ? 112 : 0;
    exec_db_x = db_state_x ? 1920 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 8064 : 0;
    exec_db_act = db_state_W ? 112 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 5 : 10;
      x_tile_size_w   = (_i_w_load+1 == 5)   ? 5 : 6;
      ...
      y_tile_size_w   = (_i_w_load+1 == 5)   ? 4 : 4;
      W_tile_size_nof = (_i_nof_load+1 == 2) ? 4 : 28;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 10, 6, 32, 20, 32,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 28, 3*3, 32, 3*3, 32, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 5648) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+9216 + 112*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 21784 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+9344 + 112*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 22016 + db_act;
    ...
    k = (int32_t *) (l1_buffer + 21784 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 22016 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 5648 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 3848 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 32 : 32;
    x_tile_size_h_exec   = (_i_h_exec+1 == 2)   ? 5 : 10;
    x_tile_size_w_exec   = (_i_w_exec+1 == 5)   ? 5 : 6;
    y_tile_size_nof = (_i_nof_exec+1 == 2) ? 4 : 28;
    y_tile_size_h   = (_i_h_exec+1 == 2)   ? 4 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 5)   ? 4 : 4;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (_i_nof_exec+1 == 2)   ? 4 : 28;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 4, 28, 20, 32, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 3848) + db_y;
