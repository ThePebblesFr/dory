...
void BNReluConvolution0(
  ...
  DMA_copy_k.loc = (uint32_t) l1_buffer + 25258;
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+928;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 25522;
  ...
  DMA_copy_x.number_of_1d_copies = 83;
  DMA_copy_x.length_1d_copy = 1;
  dory_dma_memcpy_async(&DMA_copy_x);
  dory_dma_barrier(&DMA_copy_x);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 23650) + 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 82 : 83;
      ...
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 40 : 40;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 19, 83, 1, 160, 1,  3, 3,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_W.loc = (l1_buffer + 23650) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+800 + 128*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 25258 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+928 + 128*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 25522 + db_act;
    ...
    k = (int32_t *) (l1_buffer + 25258 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 25522 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 23650 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 3162 + db_y);
    ...
    x_tile_size_w_exec   = (_i_w_exec+1 == 2)   ? 82 : 83;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 32 : 32;
    y_tile_size_h   = (_i_h_exec+1 == 6)   ? 8 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 2)   ? 40 : 40;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 40, 32, 80, 32, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 3162) + db_y;
