...
void BNReluConvolution4(
  ...
  DMA_copy_k.loc = (uint32_t) l1_buffer + 21144;
  DMA_copy_k.number_of_2d_copies = 1;
  DMA_copy_k.number_of_1d_copies = 1;
  DMA_copy_k.length_1d_copy = (uint16_t) 96;
  dory_dma_memcpy_async(&DMA_copy_k);
  dory_dma_barrier(&DMA_copy_k);


  DMA_copy_lambda.ext = (uint32_t) l2_W+18688;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 21344;
  DMA_copy_lambda.number_of_2d_copies = 1;
  DMA_copy_lambda.number_of_1d_copies = 1;
  DMA_copy_lambda.length_1d_copy = (uint16_t) 96;
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 7312) + 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      W_tile_size_nof = (_i_nof_load+1 == 3) ? 16 : 24;
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 24, 3*3, 32, 3*3, 32, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 7312) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+18432 + 96*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 21144 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+18688 + 96*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 21344 + db_act;
    ...
    k = (int32_t *) (l1_buffer + 21144 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 21344 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 7312 + exec_db_W);
    ...
    y_tile_size_nof = (_i_nof_exec+1 == 3) ? 16 : 24;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 3)   ? 16 : 24;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 5, 4, 24, 10, 64, 0, 0, 0, 0, 0, 0, 8);
